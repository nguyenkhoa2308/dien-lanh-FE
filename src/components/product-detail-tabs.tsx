...
function SpecsAccordion({ attributes }: SpecsAccordionProps) {
  ...
  if (groupedAttributes.length === 0) {
    return (
      <p className="text-gray-500 text-center py-8">Chưa có thông số chi tiết</p>
    );
  }
                ...
                <div className="px-5 pb-4">
                  {isUtility ? (
                    ...
                    <div className="divide-y divide-gray-100">
                      {section.items.map((item, idx) => (
                        <div key={idx} className="flex py-3 gap-4">
                          <span className="text-sm font-medium text-gray-700 w-2/5 flex-shrink-0">{item.label}:</span>
                          <span className={`text-sm flex-1 ${isHighlightValue(item.value) ? 'text-[#1976d2]' : 'text-gray-900'}`}>
                            {item.value}
                          </span>
                        </div>
                      ))}
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
      ...
      <div className="flex border-b border-gray-200">
        ...
        >
          Đánh giá ({formatNumber(ratingCount)})
        </button>
        <button
          type="button"
        ...
        {activeTab === 'description' && (
          <div className="markdown-content animate-fade-in">
            {description ? (
              ...
              <p className="text-gray-500 text-center py-8">Chưa có mô tả sản phẩm</p>
            )}
          </div>
        )}
